...
bool flag = true;
std::vector<std::vector<int>> x_res(2,std::vector<int> (2));
...
public: 
    ...
    void obs_sub_callback(const visualization_msgs::MarkerArray::ConstPtr& Msg){
        ...
        int k = 0;
        for(auto ptr : ptrs){
            Eigen::VectorXd res = ptr->getState();
            x_res[k][0] = res[0];
            x_res[k][1] = res[1];
            k++;
        }
        if(flag){
            for (int i = 0; i < x_res[0].size(); ++i) {
                ...
                marker.pose.position.x = x_res[k][0];
                marker.pose.position.y = x_res[k][1];
